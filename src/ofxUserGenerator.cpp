...
// handy while debuggin.
void ofxUserGenerator::drawActiveUserIDs() {
	int y = ofGetHeight()-20;
	int l = active_users.size();
	for(int i = 1; i < l; ++i) {
		if(active_users[i]) {
			glColor3f(0,1,0);
		}
		...
		ofDrawBitmapString(ofToString(i),10+i*20,y);
	}
	glColor3f(1,1,1);
}
...
//	for(int u = 0; u < 4; ++u) {
		...
		if(!is_ok) {															//  Ie.,	if userPix[i] == 0 then it's not being tracked -> it's the background!
			return;														//			if userPix[i] > 0 then the pixel belongs to the user who's value IS userPix[i]
